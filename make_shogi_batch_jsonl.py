    ...
    for move in moves:
        ...
        for comment in comments:
            if isinstance(comment, str) and comment.strip():
                block += f"手数{move_number}: {comment.strip()}\n"
    return block

def main():
    input_directory = './DataSet'
    output_jsonl = 'shogi_batch_requests.jsonl'
    requests = []
    for fname in os.listdir(input_directory):
        if not fname.endswith('.json'):
            continue
        data = load_json_file(os.path.join(input_directory, fname))
        sente = data.get('sente', '不明')
        gote = data.get('gote', '不明')
        comments_block = make_comments_block(data.get('moves', []))
        if not comments_block.strip():
            print(f"コメントがないためスキップ: {fname}")
            continue
    ...
    print(f"JSONLファイルを作成しました: {output_jsonl}")
