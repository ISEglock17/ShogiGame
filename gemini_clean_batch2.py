...
def process_shogi_json_batch(client, json_data):
    ...
    if not moves_comments:
        return None, None  # コメントがなければスキップ
    ...
    try:
        ...
        print("="*60)
        return full_response_text, prompt_text
    except Exception as e:
...
def main():
    ...
    output_directory = './ProcessedComments_reason_batch'
    ...
    for json_file in json_files:
        ...
        output_file_name = f"{base_name}_batch_processed.txt"
        ...
        if data:
            response_text, prompt_text = process_shogi_json_batch(client, data)
            if response_text is None:
                print(f"コメントがないためスキップ: {json_file}")
                continue
