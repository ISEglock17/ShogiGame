...
def main():
    ...
    memo_files = [f for f in os.listdir(input_directory) if f.endswith('.txt')]
    if not memo_files:
        print(f"'{input_directory}' ディレクトリにTXTファイルが見つかりませんでした。")
        return
    
    
    for memo_file in memo_files:
        base_name = os.path.splitext(memo_file)[0]
        print(f"ベースネームは{base_name}")
        ...
        if os.path.exists(output_path):
            print(f"スキップ中: {memo_file} - 既に整形済みファイルが存在します ({output_file_name})")
            print("-" * 30)
            continue # 次のファイルへ

        file_path = os.path.join(input_directory, memo_file)
        print(f"結果保存処理中: {file_path}")

        # メモファイルを読み、バッチジョブ名を抽出する
        try:
            with open(file_path, 'r', encoding='utf-8') as mf:
                raw_lines = [ln.strip() for ln in mf.readlines() if ln.strip()]
        except Exception as e:
            print(f"メモファイルの読み込みに失敗: {file_path}, エラー: {e}")
            print("-" * 30)
            continue

        if not raw_lines:
            print(f"メモファイルに内容がありません: {file_path}")
            print("-" * 30)
            continue

        # コメントが無い旨のマーカーがあればスキップ
        if any(line == "== コメントが1件もありません ==" for line in raw_lines):
            print(f"スキップ: {memo_file} - コメントが1件もありません")
            print("-" * 30)
            continue

        # raw_lines の中から base_name,batch_job_name の形式の行を探す
        batch_job_name = None
        for line in raw_lines:
            if ',' in line:
                parts = [p.strip() for p in line.split(',', 1)]
                # parts[0] は base_name（ファイル名と一致しているはず）、parts[1] がバッチ名
                if len(parts) >= 2 and parts[1]:
                    batch_job_name = parts[1]
                    break
            # 直接 'batches/' を含む行があればそれを使う
            if 'batches/' in line:
                batch_job_name = line
                break

        if not batch_job_name:
            print(f"バッチジョブ名がメモファイルから抽出できませんでした: {file_path}")
            print("-" * 30)
            continue
