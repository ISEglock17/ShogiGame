...
def player_turn(sfen, moves, process, response_queue, command_queue, mark_cells, pop1_se, beep_se, koma_se):
    ...
    # 合法手ごとの評価値を取得
    legal_moves_evaluations = []  # 評価値を保存するリスト
    for move in legal_moves_list:
        # 合法手を送信
        test_position_command = f"{position_command} {move}"
        send_command(process, test_position_command)
        print(f"やねうら王への送信: {test_position_command}")
        time.sleep(0.1)

        # 評価値を取得
        test_bestmove, test_comments = get_engine_move(process, response_queue)
        evaluation = extract_evaluation_value(test_comments)  # コメントから評価値を抽出する関数
        legal_moves_evaluations.append((move, evaluation))

    # 評価値付きコメントの描画
    comment_img = []
    for move, evaluation in legal_moves_evaluations:
        comment_img.append(font.render(f"手: {move}, 評価値: {evaluation}", True, (0, 0, 15)))  # 青色で描画
