...
def count_tokens_with_api(prompt_text, model_name="gemini-2.5-flash-preview-05-20"):
    # Gemini公式APIでトークン数を取得
    return genai.token_count.count_message_tokens(
        model=model_name,
...
def clean_comment_with_gemini(client, sente_name: str, gote_name: str, move_number: int, comment_text: str) -> tuple:
    ...
    # 厳密なトークン数をAPIで取得
    token_count = count_tokens_with_api(prompt_text)
    print(f"Gemini送信プロンプトのトークン数（API厳密値）: {token_count}")
    ...
    for move in json_data.get('moves', []):
        ...
        if all(isinstance(c, str) and len(c) == 1 for c in comments):
            ...
            if full_comment:
                ...
                token_info = count_tokens_with_api(prompt_text)
            ...
            for comment_item in comments:
                if isinstance(comment_item, str) and comment_item.strip():
                    prompt_text = GEMINI_PROMPT_TEMPLATE.format(
                        ...
                        gote_name=gote,
                        move_number=move_number,
                        comment_text=comment_item.strip()
                    )
                    token_info = count_tokens_with_api(prompt_text)
                    total_input_tokens += token_info.input_tokens
                    total_output_tokens += token_info.output_tokens
                    ...
                    for line in memo:
                        print(line)
                    memo_lines.extend(memo)
    return processed_lines, memo_lines, total_input_tokens, total_output_tokens
...
def main():
    ...
    for json_file in json_files:
        ...
        if data:
            processed_comments, memo_lines, total_input_tokens, total_output_tokens = process_shogi_json(client, data)
            ...
            print(f"このファイルのGemini送信プロンプト合計入力トークン数: {total_input_tokens}")
            print(f"このファイルのGemini送信プロンプト合計出力トークン数: {total_output_tokens}")
        print("-" * 30)
