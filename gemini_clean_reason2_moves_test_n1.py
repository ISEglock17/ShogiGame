...
def process_shogi_json(client, json_data, batch_size=10):
    ...
    if batch_size == 1:
        ...
        for move_number, comment_text in moves_comments:
            ...
            print(prompt_text)
            print("="*60)
            cleaned_comment, memo = clean_comment_with_gemini(client, sente, gote, move_number, comment_text)
            # memo_linesにはプロンプトとAI出力が含まれる
            for line in memo:
                print(line)
            if cleaned_comment:
        ...
        for i in range(0, len(moves_comments), batch_size):
            ...
            # バッチプロンプトはclean_comments_batch_with_gemini内でprintされる
...
def main():
    ...
    output_directory = './ProcessedComments_reason_moves_test'
    memo_directory = './ProcessedComments_reason_memo_moves_test'
    ...
    for batch_size in batch_sizes:
        ...
        # バッチサイズを渡せるようにprocess_shogi_jsonを修正
        processed_comments, memo_lines = process_shogi_json(client, data, batch_size=batch_size)
        base_name = os.path.splitext(json_file)[0]
        output_file_name = f"{base_name}_n{batch_size}_processed.txt"
        output_path = os.path.join(output_directory, output_file_name)
        memo_file_name = f"{base_name}_n{batch_size}_memo.txt"
